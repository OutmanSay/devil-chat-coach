...
教练不直接甩答案，而是走一套固定的诊断流程：

```
  你贴来一句局面     "她已读不回，我要不要再发一条？"
        │
        ▼
  ①  定位      你在哪个阶段（开场│聊天│邀约│约会│危机）＋ 对方多热（搭理→倾诉→关注→依顺）
        │
        ▼
  ②  诊断      你的草稿哪会翻车？／对方这句是好感、拒绝、还是测试？
        │
        ▼
  ③  开方子    给 2-3 个回复（稳妥／进取／调情），每个附"用了什么招、为什么"
        │
        ▼
  ④  拦雷区    "先别！再追发只会把『对你没感觉』变成『你这人没分寸』"
        │
        ▼
  你挑一条，发出去 ✅
```
